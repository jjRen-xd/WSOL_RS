...
from .Non_local import Self_Attn
...
def adjust_pretrained_model_NL(pretrained_weights, current_model):
    pretrained_list = pretrained_weights.keys()
    # [28, 26, 24, 21, 19, 17, 14, 12, 10, 7, 5, 2, 0]
    for i, layer_name in enumerate(pretrained_list):
        ...
        if layer_type == 'weight':
            assert current_model.features[layer_num].weight.data.size() == pretrained_weights[
                layer_name].size(), "size error!"
            current_model.features[layer_num].weight.data = pretrained_weights[layer_name]
        else:  # layer type == 'bias'
            assert current_model.features[layer_num].bias.size() == pretrained_weights[layer_name].size(), "size error!"
            current_model.features[layer_num].bias.data = pretrained_weights[layer_name]
...
def load_pretrained_model(model, architecture_type, is_NL=False, path=None):
    if path is not None:
    ...
    if is_NL == True:
        state_dict = adjust_pretrained_model_NL(state_dict, model)
    else:
...
def make_layers(cfg, is_NL, **kwargs):
    layers = []
    in_channels = 3
    for v in cfg:
        if v == 'M1':
...
def vgg16(architecture_type, pretrained=False, pretrained_path=None,
    ...
    layers = make_layers(configs_dict[architecture_type][config_key], is_NL=False, **kwargs)
    model = {
        'gradcampp': VggCam,
        'cam': VggCam,
        'acol': VggAcol,
    ...
    # print(model)
    if pretrained:
        model = load_pretrained_model(model, architecture_type, is_NL=False,
